...
// tag::known-issue-120[]
[discrete]
.Alert summary failures when migrating to 8.8.0 from 8.6 or earlier
[%collapsible]
====
*Details* +
If you have {elastic-sec} or {observability} rules that send alert summaries, do not upgrade {kib} to 8.8.0. Upgrade to 8.8.1 instead.

There is an initialization bug in the alerting plugin that causes impacted {kib} nodes to fail to send the summary notifications.
If the action frequency is `for each alert` instead of `summary of alerts`, the action is not affected by this bug.
