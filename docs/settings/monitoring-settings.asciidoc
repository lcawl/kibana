...
enable data collection. If you are using the {stack-security-features}, you must
be signed in as a user with the `cluster:manage` privilege to enable
...
Set to `true` (default) to enable {monitor-features} in {kib}. Unlike the
