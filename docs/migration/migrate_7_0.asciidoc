...
*Impact:* This setting is no longer necessary. If you enable {monitor-features}
across the {stack}, a monitoring agent runs on each {es} node, {ls} node, {kib}
instance, and Beat to collect and index metrics. Each node and instance is
